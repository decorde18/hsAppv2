...
function UniformSeasonPlayerCreateForm({ uniformToEdit = {}, onCloseModal }) {
  ...
  const { isCreatingUniformSeasonPlayer, createUniformSeasonPlayer } =
    useCreateUniformSeasonPlayers();
  const { isUpdating, updateUniformSeasonPlayer } =
    useUpdateUniformSeasonPlayers();
  ...
  useEffect(
    function () {
      isUpdating ||
      isCreatingUniformSeasonPlayer ||
      isLoadingUniformSeasonPlayers
        ? setIsWorking(true)
        : setIsWorking(false);
    },
    [isCreatingUniformSeasonPlayer, isUpdating]
  );

  function onSubmit(data) {
    if (isEditSession) {
      updateUniformSeasonPlayer(
        { newUniformSeasonPlayerData: data, id: editId },
        {
          onSuccess: (data) => {
            reset();
            onCloseModal?.();
  ...
  return (
    ...
    >
      <Row>
        <Heading as="h2">UniformSeasonPlayer Information</Heading>
      ...
      <FormRow>
        ...
        <Button disabled={isWorking}>
          {isEditSession
            ? 'Edit UniformSeasonPlayer'
            : 'Create New UniformSeasonPlayer'}
        </Button>
      </FormRow>
    </Form>
  );
...
export default UniformSeasonPlayerCreateForm;
